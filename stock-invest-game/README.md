...
## P0 可靠性 + 價格可信度 + 防呆（v0.2.6）

### 來源嘗試鏈與失敗可見性

- 每次取價顯示完整來源嘗試鏈（TWSE → Yahoo → Stooq），成功/失敗一目了然
- 失敗原因分類清楚：HTTP429(頻率限制)、CORS/網路錯誤、超時、無價格資料
- UI 新增「取價鏈」列，顯示最近一次取價的各來源結果
- 買入取價失敗時，日誌記錄完整嘗試鏈，alert 提示改用「手動價買入」

### priceType 標籤（每 lot）

- 現價旁固定顯示 priceType badge：`TRADE`（綠色）/ `MID` / `ASK1` / `BID1`（黃色）
- 非 TRADE 時，lot 卡片與庫存彈窗以淡黃底提示「推估價（來源-priceType），非實際成交價」
- 來源與 priceType 一致呈現（如 `TWSE-MID`）

### 交易防呆

| 防呆項 | 機制 |
|--------|------|
| 同一 tick 不重複賣出 | 每輪 tick 循環維護已賣出 lot 集合，同一 lot 不會在同一輪被重複觸發 |
| 手動賣出單筆 debounce | 每筆 lot 的「賣出」按鈕 1 秒內防重複觸發 |
| 賣出全部 debounce | 「賣出全部」按鈕 1 秒內防重複觸發，同時禁用 lots 面板內的賣出全部按鈕 |
...
## TWSE 無成交價 fallback（v0.2.4）

部分股票（如 6739）在 TWSE API 回傳成交價 `z='-'`，導致被判「查無價格」無法下單。

### Fallback 規則

當 TWSE 成交價 `z` 無效（`'-'` 或非數字）時，依序嘗試：

| 優先序 | 來源 | 欄位 | priceType |
|--------|------|------|-----------|
| 1 | 成交價 | `z` | TRADE |
| 2 | 昨收價 | `y` | TRADE |
| 3 | 最佳賣價+買價中間 | `(a1+b1)/2` | MID |
| 4 | 最佳賣價第一檔 | `a` 第一檔 | ASK1 |
| 5 | 最佳買價第一檔 | `b` 第一檔 | BID1 |

- 以上任一可用即視為有效價格，來源標示為 `TWSE-ASK1`、`TWSE-MID`、`TWSE-BID1`
- 成交價正常時顯示 `TWSE`（priceType=TRADE）

### 風險標示

- 非 TRADE 價格在 lot 看板、庫存查詢、交易日誌中標註「推估」或「委買委賣推估價」
- 避免使用者誤以為是實際成交價

### 影響範圍

- `validateStock` 接受 fallback price，不再因無成交價而拒絕下單
- 手動買入/賣出、自動策略判斷均可使用 fallback price
- lot 看板與庫存查詢的來源欄位顯示完整 priceType
...
| 檢查項 | 條件 | 失敗行為 |
|--------|------|----------|
| 格式檢查 | 台股代號須為 4~6 位數字 | 禁止下單，提示「股號格式錯誤」 |
| 報價檢查 | 至少一個即時來源（TWSE/Yahoo/Stooq）可回傳有效價格 | 禁止下單，提示「查無報價」 |

- 驗證期間按鈕顯示「驗證中…」並暫時禁用，防止重複送出
- 即使手動輸入買入價，股號仍須通過驗證

### 監控中可繼續買入

- 買入列新增獨立「買入股號」欄位，監控中不受主股號鎖定影響
- 手動買入成功後，新 lot 立即加入策略追蹤（holding phase）
- 若監控未啟動，買入後自動開始監控（既有行為保留）
- 日誌格式：`[股號][Lot#N] 手動買入 N 股 @ $X`

### 不影響既有規則

- 多 lot 分股號比價、無現價不交易、停止監控不賣出等規則維持不變

## 交易日誌強化（v0.2.2）

### 日誌格式

每筆日誌統一為 `[HH:MM:SS] [股號][Lot#N] 動作 ...` 格式，避免多股號/多 lot 混淆。
...
| 股號 | `[2330]` 對應股票代號 |
| Lot# | `[Lot#3]` 倉位 ID |
| 動作 | 手動買入 / 進入停利模式 / 停損賣出 / 移動停利賣出 / 手動賣出 / 停止監控 等 |
| 價格與股數 | `N 股 @ $X` |
| 損益/報酬 | 賣出時顯示：`報酬 +X.XX%　損益 +$N` |

### 日誌示例

```
[09:15:30] [2330][Lot#1] 手動買入 1000 股 @ $580
[10:02:15] [2330][Lot#1] 進入停利模式（現價 $640，買入 $580）
[10:45:22] [2317][Lot#2] 手動買入 500 股 @ $120
[11:20:05] [2330][Lot#1] 移動停利賣出 1000 股 @ $650　報酬 +12.07%　損益 +$70000
[11:35:10] [2317][Lot#2] 停損賣出 500 股 @ $107　報酬 -10.83%　損益 -$6500
```

### 日誌持久化

- 新增 `logEntries` 陣列存入 localStorage（最多 200 筆），每筆含 `ts`、`msg`、`type`
- 重新載入後，歷史日誌完整保留 lotId + stock 資訊
- 舊版存檔若無 `logEntries`，自動從 `trades` 陣列重建賣出記錄（容錯顯示 `stock='—'`）

### 顏色規則

- 漲（賣出獲利）：綠色
- 跌（賣出虧損）：紅色
- 中性事件（買入、進停利、停止監控等）：灰藍色
...
- 「已載入上次庫存」提示改為多股號正確表達：
  - 單一股號：顯示該股號 + 持倉資訊
  - 多股號：顯示筆數、檔數與股號列表（超過 3 檔則截斷顯示 `+N 檔`）
  - 例：`已載入上次庫存（3 筆持倉，2 檔股票：2330、2317，共 3000 股）`
- 全域摘要與庫存彈窗已為中性描述，無需額外修正
- 不影響交易邏輯，localStorage 舊資料相容
...
| trades | 交易記錄（最多保留 100 筆），每筆含 buyPrice、sellPrice、pnl、returnPct、reason、shares、buySource、lotId、stock、time |
| logEntries | 日誌記錄（最多保留 200 筆），每筆含 ts、msg、type |
